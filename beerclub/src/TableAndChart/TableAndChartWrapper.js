...
function TableAndChartWrapper() {
    ...
    useEffect(() => {
        //Moved dependencies into useEffect to remove missing dependencies warning
        ...
        //Logging Beer types
        const getBeerTypes = () => {
            let beerClubBeerTypes = [];
            for(let i=1;i<csvArr.length;i++) {
                let currBeerClubTypeObj = {};
                currBeerClubTypeObj.person = csvArr[i].split(',')[0];
                currBeerClubTypeObj.type = csvArr[i].split(',')[1];
                beerClubBeerTypes.push(currBeerClubTypeObj);
            }
            return beerClubBeerTypes;
        }
  ...
  return (
    <div className="TableAndChartWrapper">
        ...
        {console.log('Beer Types: ', beerTypes)}
